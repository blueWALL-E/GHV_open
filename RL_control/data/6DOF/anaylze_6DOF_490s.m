 ...
 * @LastEditTime: 2026-04-06 23:17:03
...
function calculate_performance_metrics( ...
    ...
    if strcmp(sigName, 'aero_ang_e')
        ...
        mean_abs_GS_neg = mean(abs(y_GS_neg(:, alpha_idx)));
        mean_abs_GS_norm = mean(abs(y_GS_norm(:, alpha_idx)));
        mean_abs_GS_pos = mean(abs(y_GS_pos(:, alpha_idx)));

        mean_abs_TD3_neg = mean(abs(y_TD3_neg(:, alpha_idx)));
        mean_abs_TD3_norm = mean(abs(y_TD3_norm(:, alpha_idx)));
        mean_abs_TD3_pos = mean(abs(y_TD3_pos(:, alpha_idx)));

        fprintf('Performance metrics for alpha channel (aero_ang_e):\n');
        fprintf('GS-negative: Max = %.4f, Sum = %.4f, Mean = %.4f\n', ...
            max_abs_GS_neg, sum_abs_GS_neg, mean_abs_GS_neg);

        fprintf('GS-normal:   Max = %.4f, Sum = %.4f, Mean = %.4f\n', ...
            max_abs_GS_norm, sum_abs_GS_norm, mean_abs_GS_norm);

        fprintf('GS-positive: Max = %.4f, Sum = %.4f, Mean = %.4f\n', ...
            max_abs_GS_pos, sum_abs_GS_pos, mean_abs_GS_pos);

        fprintf('TD3-negative: Max = %.4f, Sum = %.4f, Mean = %.4f\n', ...
            max_abs_TD3_neg, sum_abs_TD3_neg, mean_abs_TD3_neg);

        fprintf('TD3-normal:   Max = %.4f, Sum = %.4f, Mean = %.4f\n', ...
            max_abs_TD3_norm, sum_abs_TD3_norm, mean_abs_TD3_norm);

        fprintf('TD3-positive: Max = %.4f, Sum = %.4f, Mean = %.4f\n', ...
            max_abs_TD3_pos, sum_abs_TD3_pos, mean_abs_TD3_pos);
    end
