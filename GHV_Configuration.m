 ...
 * @LastEditTime: 2025-05-26 20:51:51
...
GHV_cfg.Jx = 1.36 * 10 ^ 6; %单位 kg*m^2 机体x轴的转动惯量
GHV_cfg.Jy = 1.5 * 10 ^ 7; %单位 kg*m^2 机体x轴的转动惯量
GHV_cfg.Jz = 1.36 * 10 ^ 7; %单位 kg*m^2 机体x轴的转动惯量
GHV_cfg.J = diag([GHV_cfg.Jx, GHV_cfg.Jy, GHV_cfg.Jz]); %单位 kg*m^2 机体转动惯量矩阵
Simulink.Bus.createObject(GHV_cfg); %名字是slBus1

%飞行器初始状态
Position_init = [0; 0; -50000]; % 初始位置 大地坐标系
Euler_init = [0; 0; 0]; %初始姿态 欧拉角 机体坐标系
Omega_init = [0; 0; 0]; %初始角速度 机体坐标系
Speed_init = [3000; 0; 0]; %初始速度 机体坐标系
